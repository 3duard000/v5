 ...
 * Application Processing Panel - Panel.gs
 * 
...
const Panel = {
  ...
  processApplication(applicationData) {
    try {
      ...
      // Mark the application as processed in the responses sheet
      this._markApplicationAsProcessed(data.timestamp, data.fullName);
  ...
  _getTenantApplications() {
    try {
      ...
      // Look for the form responses sheet (Google Forms creates sheets with "Form Responses" in the name)
      const sheets = spreadsheet.getSheets();
      let responseSheet = null;
      
      // Find the tenant application responses sheet
      for (let sheet of sheets) {
        const sheetName = sheet.getName().toLowerCase();
        if (sheetName.includes('tenant application') || 
            (sheetName.includes('form responses') && sheetName.includes('1'))) {
          responseSheet = sheet;
          break;
        }
      }
      ...
      if (!responseSheet) {
        // If no specific sheet found, look for any "Form Responses" sheet
        responseSheet = spreadsheet.getSheetByName('Form Responses 1');
      }
      ...
      for (let i = 1; i < data.length; i++) {
        ...
        if (isProcessed) {
          processedCount++;
          console.log(`Row ${i + 1}: Skipping processed application - ${this._getFieldValue(row, headerMap.fullName)}`);
        ...
        const application = {
          ...
          monthlyIncome: this._getFieldValue(row, headerMap.monthlyIncome) || '', // FIXED: Now properly gets the value
...
<body>
            ...
            if (confirm('Are you sure you want to reject this application? This action cannot be undone.')) {
                ...
                google.script.run
                    .withSuccessHandler(function() {
                        showStatus('Application rejected and marked as processed.', 'error');
  ...
  /**
   * Mark application as processed in the responses sheet - IMPROVED
   * @private
   */
  _markApplicationAsProcessed(timestamp, applicantName) {
    try {
      const spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
      ...
      if (processedCol === -1) {
        console.log('Could not find Processed column');
        return;
      }
      
      // Find and mark the specific application as approved
      for (let i = 1; i < data.length; i++) {
        if (data[i][0].toString() === timestamp.toString()) {
          const approvedValue = 'Approved';
          tenantSheet.getRange(i + 1, processedCol + 1).setValue(approvedValue);
          console.log(`Marked application as approved: ${applicantName} - ${approvedValue}`);
          break;
        }
      }
  ...
  /**
   * Mark application as rejected in the responses sheet
   * @private
   */
  _markApplicationAsRejected(timestamp, applicantName) {
    try {
      const spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
      const tenantSheet = spreadsheet.getSheetByName('Tenant Application');
      
      if (!tenantSheet) {
        console.log('Could not find "Tenant Application" sheet to mark as rejected');
        return;
      }
      ...
      // Find and mark the specific application as rejected
      for (let i = 1; i < data.length; i++) {
        if (data[i][0].toString() === timestamp.toString()) {
          const rejectedValue = 'Rejected';
          tenantSheet.getRange(i + 1, processedCol + 1).setValue(rejectedValue);
          console.log(`Marked application as rejected: ${applicantName} - ${rejectedValue}`);
          break;
        }
      }
...
function markApplicationAsRejected(applicationData) {
  const data = JSON.parse(applicationData);
  return Panel._markApplicationAsRejected(data.timestamp, data.fullName);
}
