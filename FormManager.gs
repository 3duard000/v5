 ...
 * Updated with clean field types and immediate sheet renaming.
 */
...
const FormManager = {
  ...
  createGoogleForms: function() {
    ...
    try {
      ...
      // Immediately create dummy response and rename
      this._forceSheetCreationAndRename(applicationForm, 'Tenant Application');
      ...
      // Immediately create dummy response and rename
      this._forceSheetCreationAndRename(moveOutForm, 'Move-Out Requests');
      ...
      // Immediately create dummy response and rename
      this._forceSheetCreationAndRename(guestCheckInForm, 'Guest Check-Ins');
      ...
      console.log('   1. All response sheets have been automatically renamed');
      console.log('   2. File upload questions must be added manually to the Tenant Application form');
      console.log('   3. Submit test responses to trigger the Processed column creation');
  ...
  /**
   * Force sheet creation by submitting dummy response, then rename and cleanup
   * @private
   */
  _forceSheetCreationAndRename: function(form, desiredName) {
    try {
      console.log('Forcing sheet creation for: ' + form.getTitle());
      
      const spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
      
      // Get current sheet names to identify new sheet
      const sheetsBefore = [];
      const sheets = spreadsheet.getSheets();
      for (let i = 0; i < sheets.length; i++) {
        sheetsBefore.push(sheets[i].getName());
      }
      
      // Create and submit dummy response
      const formResponse = form.createResponse();
      const items = form.getItems();
      
      console.log('Adding dummy data to ' + items.length + ' form items...');
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        try {
          ...
          console.log('Skipped item ' + (i + 1) + ': ' + itemError.message);
        }
      }
      
      // Submit the dummy response
      formResponse.submit();
      console.log('✅ Dummy response submitted');
      ...
      for (let j = 0; j < sheetsAfter.length; j++) {
        const sheet = sheetsAfter[j];
        const sheetName = sheet.getName();
        if (sheetName.indexOf('Form Responses') === 0) {
          let isNewSheet = true;
          for (let k = 0; k < sheetsBefore.length; k++) {
            if (sheetsBefore[k] === sheetName) {
              isNewSheet = false;
              break;
            }
          }
      ...
      if (newSheet) {
        const originalName = newSheet.getName();
        ...
        console.log('✅ Renamed "' + originalName + '" to "' + finalName + '"');
        
        // Remove dummy response (row 2, since row 1 is headers)
        if (newSheet.getLastRow() > 1) {
          newSheet.deleteRow(2);
          console.log('✅ Removed dummy response data');
        }
        
        return finalName;
        
      } else {
        console.log('❌ Could not find newly created sheet');
        return null;
      }
      
    } catch (error) {
      console.error('Error forcing sheet creation for ' + form.getTitle() + ': ' + error.toString());
      return null;
    }
  },

  /**
   * Set up a trigger to add "Processed" column when first form response is submitted
   * @private
   */
